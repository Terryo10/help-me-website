<div>
    ...
    <div class="section campaign-detail-section section-padding">
        <div class="container">
            ...
            <div class="row gy-5">
                ...
                <div class="col-lg-8">
                    ...
                    <div class="campaign-image-container wow fadeInUp">
                        ...
                        @if($campaign->gallery)
                        @php
                        $galleryImages = is_array($campaign->gallery) ? array_filter($campaign->gallery) :
                        array_filter(explode(',', $campaign->gallery));
                        @endphp
                ...
                <div class="col-lg-4">
                    <div class="campaign-sidebar">
                        ...
                        <div class="donation-card wow fadeInRight">
                            ...
                            <button data-bs-toggle="modal" data-bs-target="#donateNowModal"
                                class="btn_theme btn_theme_active w-100 mt-3">
                            ...
                            <div class="share-buttons mt-3">
                                ...
                                <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(request()->fullUrl()) }}"
                                    target="_blank" rel="noopener" class="share-btn facebook" title="Share on Facebook">
                                    <i class="bi bi-facebook"></i>
                                </a>
                                <a href="https://twitter.com/intent/tweet?url={{ urlencode(request()->fullUrl()) }}&text={{ urlencode($campaign->title) }}"
                                    target="_blank" rel="noopener" class="share-btn twitter" title="Share on Twitter">
                                    <i class="bi bi-twitter"></i>
                                </a>
                                <a href="https://wa.me/?text={{ urlencode($campaign->title . ' ' . request()->fullUrl()) }}"
                                    target="_blank" rel="noopener" class="share-btn whatsapp" title="Share on WhatsApp">
            ...
            <!-- Donate Now Modal -->
            <div style="{{$showDonationForm ? " display: block;" : "" }}" class={{$showDonationForm ? "modal fade show"
                : "modal fade" }} id="donateNowModal" tabindex="-1" aria-labelledby="donateNowModalModalLabel"
                aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="donateNowModalModalLabel">Donate Now</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <div class="donation-form-card mt-4">
                                <form wire:submit.prevent="donate">
                                    <h4>Make a Donation</h4>

                                    <div class="amount-selection mb-3">
                                        <div class="quick-amounts">
                                            <button type="button" wire:click="$set('donationAmount', 10)"
                                                class="amount-btn {{ $donationAmount == 10 ? 'active' : '' }}">$10</button>
                                            <button type="button" wire:click="$set('donationAmount', 25)"
                                                class="amount-btn {{ $donationAmount == 25 ? 'active' : '' }}">$25</button>
                                            <button type="button" wire:click="$set('donationAmount', 50)"
                                                class="amount-btn {{ $donationAmount == 50 ? 'active' : '' }}">$50</button>
                                            <button type="button" wire:click="$set('donationAmount', 100)"
                                                class="amount-btn {{ $donationAmount == 100 ? 'active' : '' }}">$100</button>
                                        </div>
                                        <div class="custom-amount mt-2">
                                            <input type="number" wire:model="donationAmount" class="form-control"
                                                placeholder="Custom amount" min="{{ $campaign->minimum_donation }}">
                                        </div>
                                        @error('donationAmount') <div class="text-danger">{{ $message }}
                                        </div> @enderror
                                    </div>

                                    @unless(auth()->check())
                                    <div class="donor-info mb-3">
                                        <input type="text" wire:model="donorName" class="form-control mb-2"
                                            placeholder="Your name">
                                        @error('donorName') <div class="text-danger">{{ $message }}
                                        </div> @enderror

                                        <input type="email" wire:model="donorEmail" class="form-control"
                                            placeholder="Your email">
                                        @error('donorEmail') <div class="text-danger">{{ $message }}
                                        </div> @enderror
                                    </div>
                                    @endunless
                                    ...
                                    <div class="donation-comment mb-3">
                                        <textarea wire:model="comment" class="form-control"
                                            placeholder="Leave a comment (optional)" rows="3"></textarea>
                                        @error('comment') <div class="text-danger">{{ $message }}</div>
                                        @enderror
                                    </div>

                                    <div class="form-check mb-3">
                                        <input class="form-check-input" type="checkbox" wire:model="isAnonymous"
                                            id="anonymous">
                                    ...
                                    <div class="amount-selection mb-3">
                                        ...
                                        <div class="quick-amounts">
                                            <button type="button" wire:click="$set('selected_gateway', 'ecocash')"
                                                class="amount-btn {{ $selected_gateway == 'ecocash' ? 'active' : '' }}">Ecocash</button>
                                            <button type="button" wire:click="$set('selected_gateway', 'paypal')"
                                                class="amount-btn {{ $selected_gateway == 'paypal' ? 'active' : '' }}">Paypal</button>
                                            <button type="button" wire:click="$set('selected_gateway', 'stripe')"
                                                class="amount-btn {{ $selected_gateway == 'stripe' ? 'active' : '' }}">Stripe</button>
                                        </div>

                                        @error('selected_gateway') <div class="text-danger">{{ $message
                                            }}</div> @enderror
                                    </div>

                                    <button type="submit" class="btn_theme btn_theme_active w-100"
                                        wire:loading.attr="disabled">
                                        <div wire:loading.remove>Continue With ${{ $donationAmount }}
                                        </div>
                                        <div wire:loading>Processing...</div>
                                    </button>

                                    <button type="button" data-bs-dismiss="modal" class="btn btn-secondary w-100 mt-2">
                                        Cancel
                                    </button>
                                </form>
                            </div>
                            ...
                            <div class="col-md-4">
                                <div class="campaign-card-small">
                                    ...
                                    <div class="campaign-content">
                                        <h5><a href="{{ route('campaigns.show', $related->slug) }}">{{
                                                Str::limit($related->title, 50) }}</a></h5>
                                        <div class="progress-info">
                                            <span>${{ number_format($related->raised_amount) }} raised</span>
                                            <span>{{ $related->goal_percentage() }}%</span>
                                        </div>
                                        <div class="progress">
                                            <div class="progress-bar" style="width: {{ $related->goal_percentage() }}%">
                                            </div>
                                        </div>
                                    </div>
                                </div>
